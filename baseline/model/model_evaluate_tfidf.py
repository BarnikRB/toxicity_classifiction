...
def main(config_dir, config_files):
    ...
    for idx, config_path in enumerate(config_paths):
        ...
        test_loader, _ = create_dataloader(
            filepath=config.get("val_data"),  # Use train_data if val_data not specified
        ...
        preds, metrics = evaluate(model, test_loader, criterion, device)
        
        # Print metrics
        print("\nEvaluation Results:")
        print(f"F1 Score - Class 0: {metrics['f1']['0']:.4f}, Class 1: {metrics['f1']['1']:.4f}")
        print(f"Precision - Class 0: {metrics['precision']['0']:.4f}, Class 1: {metrics['precision']['1']:.4f}")
        print(f"Recall - Class 0: {metrics['recall']['0']:.4f}, Class 1: {metrics['recall']['1']:.4f}")
        
        # Save predictions if requested
        if config.get("save_predictions", False):
            # Load the dataset to get original texts
            val_data = pd.read_csv(config.get("val_data"), sep='\t', header=0, quoting=3)
            val_data['predicted'] = preds
            ...
            output_path = os.path.join(output_dir, f"predictions_{os.path.basename(config_path).split('.')[0]}.tsv")
            val_data.to_csv(output_path, sep='\t', index=False)
